...
from third_party.motionbert.lib.utils.utils_data import flip_data, crop_scale
from third_party.motionbert.lib.utils.utils_mesh import flip_thetas
from third_party.motionbert.lib.utils.utils_smpl import SMPL
from third_party.motionbert.lib.data.datareader_h36m import DataReaderH36M  
from third_party.motionbert.lib.data.datareader_mesh import DataReaderMesh  
from third_party.motionbert.lib.data.dataset_action import random_move  

# PROJ_DIR = os.path.dirname(ROOT_DIR.split('third_party')[0])
# sys.path.append(os.path.join(PROJ_DIR, 'data_gen'))
...
class MotionSMPL(SMPLDataset):
    ...
    def _load_skeleton(self):
        skeleton_info = np.load('/home/wxs/Skeleton-in-Context-tpami/data/support_data/body_models/smpl_skeleton.npz')      # (1,52,3)
        self.p3d0 = torch.from_numpy(skeleton_info['p3d0']).float()[:, :22]                                                 # (1,22,3)
        parents = skeleton_info['parents']                                                                                  # (52,)
        self.parent = {}
    ...
    def get_3d_joints(self, motion_smpl_pose):
        motion_smpl_pose = motion_smpl_pose[:, :-2]
        motion_smpl_pose = R.from_rotvec(motion_smpl_pose.reshape(-1, 3)).as_rotvec()
        motion_smpl_pose = motion_smpl_pose.reshape(-1, 22, 3)
        motion_smpl_pose[:, 0] = 0

        p3d0_tmp = self.p3d0.repeat([motion_smpl_pose.shape[0], 1, 1])
        motion_smpl_pose = ang2joint(p3d0_tmp, torch.tensor(motion_smpl_pose).float(), self.parent)
        motion_smpl_pose = motion_smpl_pose.reshape(-1, 22, 3)
        return motion_smpl_pose
    ...
    def __getitem__(self, index):
        ...
        motion_3d_joints = self.get_3d_joints(motion_smpl_pose)                      # motion_3d_joints: (T,22,3)
        ...
        motion_verts = motion_smpl.vertices.detach()*1000.0                         # [16,6890,3]
