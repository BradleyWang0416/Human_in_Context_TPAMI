...
def crop_scale_3d(motion, scale_range=[1, 1]):
    ...
    if isinstance(result, np.ndarray):
        ...
        xmin = torch.min(motion[...,0])
        xmax = torch.max(motion[...,0])
        ymin = torch.min(motion[...,1])
        ymax = torch.max(motion[...,1])
        ratio = torch.rand(1) * (scale_range[1]-scale_range[0]) + scale_range[0]
    scale = max(xmax-xmin, ymax-ymin) / ratio
    if scale==0:
        if isinstance(result, np.ndarray):
            return np.zeros(motion.shape)
        elif isinstance(result, torch.Tensor):
            return torch.zeros(motion.shape)
    xs = (xmin+xmax-scale) / 2
    ys = (ymin+ymax-scale) / 2
    if isinstance(result, np.ndarray):
        result[...,:2] = (motion[..., :2]- [xs,ys]) / scale
    elif isinstance(result, torch.Tensor):
        result[...,:2] = (motion[..., :2]- torch.tensor([xs,ys])) / scale
