...
class Trainer():
    ...
    def train(self, ):
        ...
        for i, batch in enumerate(self.train_loader):                                                       # self.train_loader.dataset = <lib.data.datasets.mixed_dataset.DataFactory object at 0x7fd6f32a6e90>
                                                                                                            # self.train_loader.dataset.datasets = [<lib.data.datasets.amass.AMASSDataset object at 0x7fd6f32b6290>]
            # <======= Feedforward 
            x, inits, features, kwargs, gt = prepare_batch(batch, self.device, self.train_stage=='stage2')  # x: [64,81,37]
            timer['data'] = time.time() - start
            start = time.time()
            pred = self.network(x, inits, features, **kwargs)                                               # self.network.__class__ = <class 'lib.models.wham.Network'>
