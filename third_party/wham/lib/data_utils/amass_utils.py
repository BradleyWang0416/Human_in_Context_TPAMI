...
def process_amass():
    ...
    for seq in (seq_bar):
        ...
        for subj in (subj_bar):
            ...
            act_bar = tqdm(sorted(acts), leave=False)
            for act in (act_bar):
                ...
                if fname.endswith('shape.npz') or fname.endswith('stagei.npz'): 
                    ...
                    continue
                data = dict(np.load(fname, allow_pickle=True))                              # data.keys() = dict_keys(['trans', 'gender', 'mocap_framerate', 'betas', 'dmpls', 'poses'])
