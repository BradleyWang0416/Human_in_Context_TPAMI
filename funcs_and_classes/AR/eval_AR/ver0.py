...
def evaluate_action_recognition(args, test_loader, model, epoch=None, if_viz=False):
    ...
    with torch.no_grad():
        for idx, (query_input, query_label, prompt_input, prompt_label) in enumerate(test_loader):
            ...
            # Scheme 1
            true_labels = targets.clone().unsqueeze(0).unsqueeze(0).unsqueeze(0)        # (1, 1, 1, 60, 3)
            pred_label = pred_target.clone().unsqueeze(3)                               # (B, 4, 25, 1, 3)
            distances = torch.norm(pred_label - true_labels, dim=-1)            # (B, 4, 25, 60)
            pred_class = distances.argmin(-1)   # (B, 4, 25)
            
            true_class = query_label.unsqueeze(-1).unsqueeze(-1).expand_as(pred_class)  # (B, 4, 25)
            correct_ratio = (pred_class == true_class).type(torch.int).sum(-1).sum(-1) / (L * J)   # (B)
            ...
            pred_label = pred_target.clone().mean(1).mean(1).unsqueeze(1)               # (B, 1, 3)
