...
class Dataset_ActionRecognition(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
                 ...
                 bone=False, vel=False, prompt_data=None, prompt_label=None, subsample_stride=4):
        """
        :param data_path:
        :param label_path:
    ...
    def find_valid_frames(self):
        valid_frames_list = []
        for b in range(len(self.data)):
            valid_frames = np.where(self.data[b].sum(0).sum(-1).sum(-1) != 0)[0]
            valid_frames_list.append(valid_frames)
        self.valid_frames_list = valid_frames_list

        if self.split == 'test':
            prompt_valid_frames_list = []
            for b in range(len(self.prompt_data)):
                prompt_valid_frames = np.where(self.prompt_data[b].sum(0).sum(-1).sum(-1) != 0)[0]
                prompt_valid_frames_list.append(prompt_valid_frames)
            self.prompt_valid_frames_list = prompt_valid_frames_list
    ...
    def get_one_sample(self, index, is_prompt=False):
        if self.split == 'test' and is_prompt:
            ...
            valid_frames = self.prompt_valid_frames_list[index]
        else:
            data_numpy = self.data[index]   # (3,300,25,2)
            label = self.label[index]       # int
            valid_frames = self.valid_frames_list[index]

        data_numpy = np.array(data_numpy)   # (3,300,25,2)


        if self.split == 'train' and is_prompt == False and len(np.arange(0, len(valid_frames)-self.window_size+1, self.subsample_stride)) >= 2:    # only apply to train query
            data_valid = data_numpy[:, valid_frames, :, :]  # (3,F,25,2)
            F = len(valid_frames)
            subdata_valid = []
            for st in range(0, F-self.window_size+1, self.subsample_stride):
                subdata_valid_ = data_valid[:, st:st+self.window_size, :, :]
                subdata_valid.append(subdata_valid_)
            data_numpy = np.stack(subdata_valid)     # (S,3,64,25,2)
        else:
            ...
            data_numpy = data_numpy[None, ...]      # (1,3,64,25,2)
        ...
        if self.split == 'test' and is_prompt:
            if index in self.prompt_indices1:
                data_numpy = data_numpy[..., [0]]  # (1,3,64,25,1)
        else:
            if index in self.indices1:
                data_numpy = data_numpy[..., [0]]   # [CASE 1: test query] (1,3,64,25,1)
                                                    # [CASE 2: train query] (S,3,64,25,1)
                                                    # [CASE 3: train prompt] (1,3,64,25,1)

        if not isinstance(data_numpy, torch.FloatTensor):
            data_numpy = torch.FloatTensor(data_numpy)

        return data_numpy.permute(0,4,2,3,1), label, index     # (S,2,64,25,3); int; int

    def __getitem__(self, index):
        query_input, query_label, query_index = self.get_one_sample(index)  # (S,M,64,25,3). S=1,2. M=1,2.
        if self.split == 'test':
        ...
        prompt_input, prompt_label, prompt_index = self.get_one_sample(prompt_index, is_prompt=True)   # (S,M,64,25,3). S=1,2. M=1,2.
        assert query_input.shape[1] == prompt_input.shape[1]    # if query and prompt are equal in M
...
def collate_func(batch):
    ...
    for b in range(len(batch)):
        ...
        # query_input: [CASE 1: train query] (S,1,64,25,3) or (S,2,64,25,3);
        #              [CASE 2: test query] (1,1,64,25,3) or (1,1,64,25,3).
        # prompt_input: [CASE 1: train prompt] (1,1,64,25,3) or (1,2,64,25,3);
        #               [CASE 2: test prompt] (1,1,64,25,3) or (1,1,64,25,3).
        M = query_input.shape[1]
        S = query_input.shape[0]
        if M == 1:
            QUERY_INPUT_1.append(query_input)       # [train] (S,1,64,25,3) or [test] (1,1,64,25,3)
            subdata_num_list_1.append(S)
            PROMPT_INPUT_1.append(prompt_input.expand_as(query_input))     # (1,1,64,25,3)
            QUERY_LABEL_1 = QUERY_LABEL_1 + [query_label]*S
            PROMPT_LABEL_1 = PROMPT_LABEL_1 + [prompt_label]*S
        else:
            QUERY_INPUT_2.append(query_input)       # [train] (S,2,64,25,3) or [test] (1,2,64,25,3)
            subdata_num_list_2.append(S)
            PROMPT_INPUT_2.append(prompt_input.expand_as(query_input))     # (1,2,64,25,3)
            QUERY_LABEL_2 = QUERY_LABEL_2 + [query_label]*S
            PROMPT_LABEL_2 = PROMPT_LABEL_2 + [prompt_label]*S

    if len(QUERY_INPUT_1) != 0:
        QUERY_INPUT_1 = torch.cat(QUERY_INPUT_1)     # (N1,1,T,J,C). N1 = /sum_{b1=0}^{b1=B1} S_{b1}
    ...
    if len(QUERY_INPUT_2) != 0:
        QUERY_INPUT_2 = torch.cat(QUERY_INPUT_2)     # (N2,2,T,J,C). N2 = /sum_{b2=0}^{b2=B2} S_{b2}
