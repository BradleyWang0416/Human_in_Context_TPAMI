...
class Dataset_ActionRecognition(Dataset):
    ...
    def split_and_pair_subdata(self):
        ...
        for b in range(len(self.data)):
            ...
            F = len(valid_frames)
            subdata_st = np.arange(0, F - self.window_size + 1, self.subsample_stride)
    ...
    def __len__(self):
        if self.split == 'train':
            return len(self.subdata_indices)    # 235690
        elif self.split == 'test':
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            query_input = self.data[query_idx]   # (3,300,25,2)
            query_label = self.label[query_idx]   # int
            query_valid_frame_num = np.sum(query_input.sum(0).sum(-1).sum(-1) != 0)
            query_input = valid_crop_resize(query_input, query_valid_frame_num, self.p_interval, self.window_size)  # (3,64,25,2)
            if query_idx in self.indices1:
                prompt_idx = np.random.choice(self.prompt_indices1)
            elif query_idx in self.indices2:
                prompt_idx = np.random.choice(self.prompt_indices2)
            prompt_input = self.prompt_data[prompt_idx]   # (3,300,25,2)
            prompt_label = self.prompt_label[prompt_idx]   # int
            assert (query_idx in self.indices1) == (prompt_idx in self.prompt_indices1)
            assert (query_idx in self.indices2) == (prompt_idx in self.prompt_indices2)
