...
def evaluate(args, test_loader, h36m_datareader, model, dataset, eval_task, epoch=None, if_viz=False, if_debug=False):
    if eval_task == 'PE':
        if dataset in ['H36M_3D']:
            e1, _, summary_table, header, results = evaluate_pose_estimation(args, model, test_loader, h36m_datareader, dataset, epoch, if_viz, if_debug)
        else:
            e1, summary_table, header, results = evaluate_pose_estimation_excl_H36M(args, test_loader, model, dataset, epoch, if_viz, if_debug)
    elif eval_task == 'FPE':
        e1, summary_table, header, results = evaluate_future_pose_estimation(args, test_loader, model, dataset, epoch, if_viz, if_debug)
    elif eval_task == 'MP':
        e1, summary_table, header, results = evaluate_motion_prediction(args, test_loader, model, dataset, epoch, if_viz, if_debug)
    elif eval_task == 'MC':
        e1, summary_table, header, results = evaluate_motion_completion(args, test_loader, model, dataset, epoch, if_viz, if_debug)
    elif eval_task == 'MIB':
        e1, summary_table, header, results = evaluate_motion_in_between(args, test_loader, model, dataset, epoch, if_viz, if_debug)
    return e1, summary_table, header, results
...
def evaluate_future_pose_estimation(args, test_loader, model, dataset, epoch=None, if_viz=False, if_debug=False):
    ...
    with torch.no_grad():
        for idx, (QUERY_INPUT, QUERY_TARGET, PROMPT_INPUT, PROMPT_TARGET, INFO) in enumerate(test_loader[(dataset, 'FPE')]):
    ...
    return mpjpe_avg, summary_table, [f'{i}' for i in frame_to_eval], list(mpjpe)
...
def evaluate_motion_completion(args, test_loader, model, dataset, epoch=None, if_viz=False, if_debug=False):
    ...
    num_samples = 0
    with torch.no_grad():
        for idx, (QUERY_INPUT, QUERY_TARGET, PROMPT_INPUT, PROMPT_TARGET, INFO) in enumerate(test_loader[(dataset, 'MC')]):
    ...
    summary_table.float_format = ".2"
    print(f"costs {time.time()-st:.2f}s")
    return mpjpe_avg, summary_table, [ratio for ratio, err in mpjpe_per_ratio.items()], [err for ratio, err in mpjpe_per_ratio.items()]
...
def evaluate_motion_in_between(args, test_loader, model, dataset, epoch=None, if_viz=False, if_debug=False):
    ...
    num_samples = 0
    with torch.no_grad():
        for idx, (QUERY_INPUT, QUERY_TARGET, PROMPT_INPUT, PROMPT_TARGET, INFO) in enumerate(test_loader[(dataset, 'MIB')]):
                ...
                if if_viz:
                    ...
                    if len(if_viz.split(',')) > 1:
                        if if_viz.split(',')[1] != dataset:
                            return 1,2,3,4
                    print(f'!!! Do visualing now in [{inspect.currentframe().f_code.co_name}]')
                    gt_velocity = np.mean(np.linalg.norm(gt[1:, :, :].cpu().numpy() * 1000 - gt[:-1, :, :].cpu().numpy() * 1000, axis=-1))
                    if gt_velocity < 20: continue
                    data_viz = {
                        'input': query_batch[i, :args.data.clip_len, :, :],
                        'pred': pred,
                        'gt': gt,
                    }
                    next_target = test_loader[(dataset, 'MIB')].dataset.__getitem__(INFO[i]['query_index'] + 1)[1].squeeze(0)
                    data_viz['next gt'] = next_target
                    viz_skel_seq_anim(data_viz, mode='img', subplot_layout=(4,1), fs=0.15, lim3d=0.4, fig_title=f'{dataset}-{idx}-{i}-mask{frame_mask_idx}-{mpjpe_}-velo{gt_velocity}',
                                        azim=90, elev=90)
...
def evaluate_motion_prediction(args, test_loader, model, dataset, epoch=None, if_viz=False, if_debug=False):
    ...
    with torch.no_grad():
        for idx, (QUERY_INPUT, QUERY_TARGET, PROMPT_INPUT, PROMPT_TARGET, INFO) in enumerate(test_loader[(dataset, 'MP')]):
...
def evaluate_pose_estimation(args, model_pos, test_loader, datareader, dataset, epoch=None, if_viz=False, if_debug=False):
    ...
    with torch.no_grad():
        for idx, (QUERY_INPUT, QUERY_TARGET, PROMPT_INPUT, PROMPT_TARGET, INFO) in enumerate(test_loader[(dataset, 'PE')]):
    ...
    for idx in range(len(action_clips)):
        ...
        if if_viz:
            ...
            viz_skel_seq_anim(data_to_viz, mode='img', subplot_layout=(2,1), fs=0.3, lim3d=1000, fig_title=f'{dataset}-{idx}')
    ...
    summary_table.float_format = ".2"
    print(f"costs {time.time()-st:.2f}s")
    return e1, e2, summary_table, action_names, final_result
