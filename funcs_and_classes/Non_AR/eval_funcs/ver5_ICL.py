...
def evaluate(args, TEST_LOADER, h36m_datareader, model, dataset, eval_task, epoch=None, if_viz=False, if_debug=False):
    ...
    with torch.no_grad():
        ...
        for idx, (query_input_dict, query_target_dict, prompt_input_dict, prompt_target_dict, info_dict) in enumerate(test_loader):
            ...
            query_input_dict = test_loader.dataset.preprocess(query_input_dict)
            prompt_input_dict = test_loader.dataset.preprocess(prompt_input_dict)
            ...
            output_joint, output_smpl, target_joint, target_smpl = model(query_input_dict, prompt_input_dict,
            ...
            if if_viz:
                ...
                if 128 not in info_dict['query_index']: continue
                ...
                if len(if_viz.split(',')) == 2 and if_viz.split(',')[1] != dataset and if_viz.split(',')[1] != eval_task:
                    return 1,2,3,4,5,6,7
                if len(if_viz.split(',')) == 3 and (if_viz.split(',')[1] != dataset or if_viz.split(',')[2] != eval_task):
                    return 1,2,3,4,5,6,7
                
                output_joint_compare, output_smpl_compare, _, _ = model(query_input_dict, prompt_input_dict,
                                                                        query_target_dict, prompt_target_dict, info_dict, epoch, vertex_x1000=args.vertex_x1000, deactivate_prompt_branch=True)

                if eval_task in ['PE', 'FPE']: 
                    assert (query_input_dict['joint'][..., -1] == 0).all()
                    query_input_dict['joint'] = query_input_dict['joint'][..., :2]
                    prompt_input_dict['joint'] = prompt_input_dict['joint'][..., :2]

                for i in range(0, batch_size, 128):
                    query_index = info_dict['query_index'][i]
                    if query_index != 128: continue
                    ...
                    #                                             ('AMASS','MC',11264),
                    #                                             ]: continue
                    prompt_input_joint_viz = prompt_input_dict['joint'][i].cpu().numpy()
                    prompt_target_joint_viz = prompt_target_dict['joint'][i].cpu().numpy()
                    ...
                    output_joint_compare_viz = output_joint_compare[i].cpu().numpy()
                    output_vertex_compare_viz = output_smpl_compare[0]['verts'][i].cpu().numpy()
                    # if torch.norm(output_joint[i, 1:] - output_joint[i, :-1], dim=-1).mean().item() < 0.01: continue
                    # if (input_joint_viz[..., 0, :] != 0).any(): print('WARNING: joint #0 isn\'t at origin')
                    data_viz = {
                        'prompt_input': prompt_input_joint_viz[::4],'prompt_target': prompt_target_joint_viz[::4],
                        'query_input': input_joint_viz[::4],        'filler1': np.zeros_like(input_joint_viz)[::4],
                        'output_joint': output_joint_viz[::4],      'output_joint_PromptDeactivated': output_joint_compare_viz[::4],
                        'target_joint': target_joint_viz[::4],      'filler2': np.zeros_like(target_joint_viz)[::4],
                        'output_vertex': output_vertex_viz[::4],    'output_vertex_PromptDeactivated': output_vertex_compare_viz[::4],
                        # 'filler': np.zeros_like(input_joint_viz)[::4],
                        'target_vertex': target_vertex_viz[::4],    'filler3': np.zeros_like(target_vertex_viz)[::4],
                        # 'output_kp3d': output_kp3d_viz[::4],
                        # 'target_kp3d': target_kp3d_viz[::4],
                    }
                    viz_skel_seq_anim(data_viz, mode='img', subplot_layout=(6,2), tight_layout=True, fig_title=f"{dataset}_{eval_task}_{idx}_{i}_queryindex{info_dict['query_index'][i]}", 
                                      if_node=True, azim=-140, elev=15,
                                      lim3d=[0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,1000,1000,1000,1000], 
                                      file_name=f'{dataset}_{eval_task}_queryindex{query_index}',
                                      file_folder='viz_results/exp050106/more',
                                      if_print=True, fs=0.5, lw=5, node_size=2,
                                    #   if_print=0, fs=0.2, lw=2, node_size=2,
                                      )
                    print(f'Viz: {dataset}_{eval_task}_{idx}_{i}_queryindex{info_dict["query_index"][i]}')
                    # exit(0)
                    if query_index == 128: return 1,2,3,4,5,6,7
