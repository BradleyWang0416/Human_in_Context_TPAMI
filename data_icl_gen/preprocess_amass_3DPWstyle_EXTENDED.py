...
for split in dataset_id_dict:
    ...
    for dataset_name in dataset_id_dict[split]:
        ...
        if save_vertices:
            smpl_vertices_w_GlobalOrient = []
            smpl_vertices_wo_GlobalOrient = []
        smpl_pose = []
        smpl_shape = []
        source = []
        ...
        for i in tqdm(id_list):
            ...
            for sid in range(num_slice+1):
                ...
                if save_vertices:
                    smpl_vertices_w_GlobalOrient.append(vertices_w_GlobalOrient)
                    smpl_vertices_wo_GlobalOrient.append(vertices_wo_GlobalOrient)
        ...
        if save_vertices:
            smpl_vertices_w_GlobalOrient = np.concatenate(smpl_vertices_w_GlobalOrient)
            smpl_vertices_wo_GlobalOrient = np.concatenate(smpl_vertices_wo_GlobalOrient)
        smpl_pose = np.concatenate(smpl_pose)
        smpl_shape = np.concatenate(smpl_shape)

        all_dict = {
                'joint_3d_w_GlobalOrient': joint_3d_w_GlobalOrient,
        ...
        if save_vertices:
            all_dict['smpl_vertices_w_GlobalOrient'] = smpl_vertices_w_GlobalOrient
            all_dict['smpl_vertices_wo_GlobalOrient'] = smpl_vertices_wo_GlobalOrient

        if not os.path.exists(f'data_icl_gen/processed_data/AMASS/EXTENDED_DATA/{split}/{dataset_name}'):
            os.makedirs(f'data_icl_gen/processed_data/AMASS/EXTENDED_DATA/{split}/{dataset_name}')
        save_path = f'data_icl_gen/processed_data/AMASS/EXTENDED_DATA/{split}/{dataset_name}/amass_fps60_JOINTS_SMPL.pkl'
