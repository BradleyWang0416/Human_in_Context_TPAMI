...
def viz_skel_seq_anim(data, mode='animation', if_node=False, if_origin=False, if_target=False, 
                      ...
                      show_axis=True, transparent_background=False, tight_layout=False,
    ...
    colors_list = colors_list + [{"lcolor": 'gray', "rcolor": 'black'} for _ in range(100)]
    ...
    for idx, (key, value) in enumerate(init_pose.items()):
        ...
        if mode == 'img':
            for frame in range(frame_n):
                ...
                if ax.name == '3d':
                    if isinstance(lim3d, int) or isinstance(lim3d, float):
                        ax.set_xlim3d([-1*lim3d, 1*lim3d])
                        ax.set_ylim3d([-1*lim3d, 1*lim3d])
                        ax.set_zlim3d([-1*lim3d, 1*lim3d])
                    elif isinstance(lim3d, list):
                        ax.set_xlim3d([-1*lim3d[idx], 1*lim3d[idx]])
                        ax.set_ylim3d([-1*lim3d[idx], 1*lim3d[idx]])
                        ax.set_zlim3d([-1*lim3d[idx], 1*lim3d[idx]])
                    ax.set_zlabel('Z')
                    if isinstance(azim, int) or isinstance(azim, float):
                        ax.view_init(azim=azim, elev=elev)
                    elif isinstance(azim, list):
...
def create_pose(ax, plot, pose, colors, label=None, update=False, if_node=False, if_origin=False, lw=2, node_size=5, special_node=None, special_edge=None):
    ...
    if if_node == False and if_origin == False:
        ...
        if special_node is not None and special_node < len(node_colors):
