...
class Feeder(Dataset):
    ...
    def get_one_sample(self, index, is_test_prompt=False):
        if self.split == 'test' and is_test_prompt:
    ...
    def __getitem__(self, index):
        query_input, query_label, query_index = self.get_one_sample(index)  # (3,64,25,2)
        if self.split == 'test':
            prompt_index = np.random.randint(len(self.prompt_label))
        else:
            prompt_index = np.random.randint(len(self.label))
        prompt_input, prompt_label, prompt_index = self.get_one_sample(prompt_index, is_test_prompt=True)
...
class Feeder_NEW(Dataset):
    def __init__(self, data_path, label_path=None, p_interval=1, split='train', random_choose=False, random_shift=False,
        ...
        if split == 'test':
            self.prompt_data = prompt_data      # equal to Feeder_NEW(split='train').data
            self.prompt_label = prompt_label    # equal to Feeder_NEW(split='train').label
    ...
    def find_indices(self):
        indices1 = np.where((self.data[..., -1] == 0).all(-1).all(-1).all(-1) == True)[0]
        indices2 = np.setdiff1d(np.arange(len(self.label)), indices1)
        self.indices1 = indices1
        self.indices2 = indices2
        if self.split == 'test':
            self.prompt_indices1
    ...
    def get_one_sample(self, index, is_test_prompt=False):
        ...
        return data_numpy, label, index     # (3,64,25,2); int; int
