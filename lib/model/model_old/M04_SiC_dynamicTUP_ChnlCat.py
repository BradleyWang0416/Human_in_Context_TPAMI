...
class Skeleton_in_Context(nn.Module):
    def __init__(self, args, dim_in=3, dim_out=3, dim_feat=512, dim_rep=512, depth=5, num_heads=8, mlp_ratio=2,
        ...
        self.blocks_st = nn.ModuleList([
            Block(
                dim=dim_feat*2, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.blocks_ts = nn.ModuleList([
            Block(
                ...
                st_mode="stage_ts")
            for i in range(depth)])
        self.norm = norm_layer(dim_feat*2)
        if dim_rep:
            self.pre_logits = nn.Sequential(OrderedDict([
                ('fc', nn.Linear(dim_feat*2, dim_rep)),
                ('act', nn.Tanh())
            ]))
        else:
        ...
        self.att_fuse = att_fuse
        if self.att_fuse:
            self.ts_attn = nn.ModuleList([nn.Linear(dim_feat*2*2, 2) for i in range(depth)])
            for i in range(depth):
    ...
    def forward(self, Prompt, Query, epoch=None):
        ...
        Prompt_embed = self.encode(Prompt)  # (B,F,17,dim_feat)
        ...
        x = torch.cat((Prompt_embed, Query_embed), dim=-1)  # (B,F,17,dim_feat*2)
        x = x.reshape(B * F, J, self.dim_feat*2)  # (B*F,17,dim_feat*2)

        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            x_st = blk_st(x, F)     # (B*F,17,dim_feat*2)
            x_ts = blk_ts(x, F)     # (B*F,17,dim_feat*2)
            if self.att_fuse:
                att = self.ts_attn[idx]     # nn.Linear(dim_feat*2*2, 2)
                alpha = torch.cat([x_st, x_ts], dim=-1)     # (B*F,17,dim_feat*2*2)
                BF, J = alpha.shape[:2]
                alpha = att(alpha)  # (B*F,17,2)
                alpha = alpha.softmax(dim=-1)   # (B*F,17,2)
                x = x_st * alpha[:,:,0:1] + x_ts * alpha[:,:,1:2]   # (B*F,17,dim_feat*2)
            else:
                x = (x_st + x_ts)*0.5
        x = self.norm(x)            # (B*F,17,dim_feat*2)
        x = x.reshape(B, F, J, -1)  # (B,F,17,dim_feat*2)
