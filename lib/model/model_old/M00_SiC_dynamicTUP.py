...
class Skeleton_in_Context(nn.Module):
    ...
    def forward(self, Prompt, Query, epoch=None):
        ...
        Prompt_embed = self.encode(Prompt)  # (B,F,17,512)
        Query_embed = self.encode(Query)  # (B,F,17,512)
        ...
        Query_embed = Query_embed * (1 - m) + TUP * m   # (B,F,17,512)

        x = torch.cat((Prompt_embed, Query_embed), dim=0)  # (2B,F,17,512)
        x = x.reshape(2 * B * F, J, self.dim_feat)  # (2B*F,17,512)

        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            if idx == self.merge_idx:
                x = x.reshape(2 * B,  F, J, self.dim_feat)  # (2B,F,17,512)
                x = (x[:B] + x[B:]) * 0.5   # (B,F,17,512)
                x = x.reshape(B * F, J, self.dim_feat)  # (BF,17,512)
            x_st = blk_st(x, F)     # (2B*F,17,512) -- before merge; (B*F,17,512) -- after merge
            x_ts = blk_ts(x, F)     # (2B*F,17,512) -- before merge; (B*F,17,512) -- after merge
            if self.att_fuse:
                ...
                x = (x_st + x_ts)*0.5
        x = self.norm(x)            # (B*F,17,512)
        x = x.reshape(B, F, J, -1)  # (B,F,17,512)
        ...
        x = self.pre_logits(x)      # (B,F,17,512) --Linear(dim_feat=512, dim_rep=512)--> --tanh--> (B,F,17,512)

        if if_ntu60 and self.merge_double_action == 'before_head':
            x = x.reshape(B_, M, F, J, -1)
            x = x.mean(1, keepdim=True) 
        x = self.head(x)            # (B,F,17,512) --Linear(512,3)--> (B,F,17,3)
