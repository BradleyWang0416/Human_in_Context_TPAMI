...
class Skeleton_in_Context_OneStage(MixSTE2):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4, num_heads=8, mlp_ratio=2, 
                 qkv_bias=True, qk_scale=None, drop_rate=0, attn_drop_rate=0, drop_path_rate=0.2, norm_layer=None, is_train=True, 
    ...
    def ST_foward(self, x, residual_x=None):
        assert len(x.shape)==4, "shape is equal to 4"
        b, f, n, cw = x.shape
        for i in range(1, self.block_depth):
            steblock = self.STEblocks[i]
            ...
            if residual_x is not None:
                if self.fuse_prompt_query == 'add':
                    ...
                    x = self.fuse_query_prompt[i+3](x, residual_x[i-1])
    ...
    def forward(self, Prompt, Query, epoch=None, apply_mask=False):                    # query_input: (B,243,17,3)
        B, F, J, C = Query.shape
        F1 = F // 2
        ...
        # Apply random mask on query_input along the second dimension
        if self.training and apply_mask:
            mask = torch.rand(query_input.shape[0], query_input.shape[1], query_input.shape[2], query_input.shape[3], device=query_input.device) > 0.5
            query_input = query_input * mask
        ...
        query = self.ST_foward(query, PROMPTS[4:])
...
class Skeleton_in_Context(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        self.stage1 = Skeleton_in_Context_OneStage(num_frame, num_joints, in_chans, embed_dim_ratio, depth, num_heads, mlp_ratio,
        ...
        self.stage2 = Skeleton_in_Context_OneStage(num_frame, num_joints, in_chans, embed_dim_ratio, depth, num_heads, mlp_ratio,
                                                    qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate, norm_layer, is_train,
                                                    prompt_enabled, sqrt_alphas_cumprod, sqrt_one_minus_alphas_cumprod,
                                                    prompt_gt_as_condition, use_text, fuse_prompt_query, max_clip_len)
    
    def forward(self, Prompt, Query, epoch=None):
        query_recon = self.stage1(Prompt, Query, epoch, apply_mask=True)
        query_out = self.stage2(Prompt, torch.cat([query_recon,query_recon], dim=-3), epoch, apply_mask=False)
        if self.training:
            return query_out, query_recon
        else:
            return query_out