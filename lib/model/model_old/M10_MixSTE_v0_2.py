...
class  MixSTE2(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
        ...
        self.STEblocks = nn.ModuleList([
            ...
            Block(
                dim=embed_dim_ratio*2, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[2], norm_layer=norm_layer),
            Block(
                dim=embed_dim_ratio*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
            ...
            Block(
                dim=embed_dim_ratio*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[5], norm_layer=norm_layer),
            Block(
                dim=embed_dim_ratio*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[6], norm_layer=norm_layer),
            Block(
                dim=embed_dim_ratio*2, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[7], norm_layer=norm_layer),
            ])
        ...
        self.TTEblocks = nn.ModuleList([
            ...
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[1], norm_layer=norm_layer, comb=False, changedim=False, currentdim=2, depth=depth),
            Block(
                dim=embed_dim*2, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[2], norm_layer=norm_layer, comb=False, changedim=False, currentdim=3, depth=depth),
            Block(
                dim=embed_dim*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[3], norm_layer=norm_layer, comb=False, changedim=False, currentdim=4, depth=depth),
            Block(
                dim=embed_dim*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[4], norm_layer=norm_layer, comb=False, changedim=False, currentdim=5, depth=depth),
            Block(
                dim=embed_dim*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[5], norm_layer=norm_layer, comb=False, changedim=False, currentdim=6, depth=depth),
            Block(
                dim=embed_dim*4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[6], norm_layer=norm_layer, comb=False, changedim=False, currentdim=7, depth=depth),
            Block(
                dim=embed_dim*2, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[7], norm_layer=norm_layer, comb=False, changedim=False, currentdim=8, depth=depth)
            ])

        self.Spatial_norm = nn.ModuleList([norm_layer(embed_dim_ratio),
                                           norm_layer(embed_dim_ratio),
                                           norm_layer(embed_dim_ratio*2),
                                           norm_layer(embed_dim_ratio*4),norm_layer(embed_dim_ratio*4),norm_layer(embed_dim_ratio*4),
                                           norm_layer(embed_dim_ratio*4),
                                           norm_layer(embed_dim_ratio*2),])
        self.Temporal_norm = nn.ModuleList([norm_layer(embed_dim_ratio),
                                           norm_layer(embed_dim_ratio),
                                           norm_layer(embed_dim_ratio*2),
                                           norm_layer(embed_dim_ratio*4),norm_layer(embed_dim_ratio*4),norm_layer(embed_dim_ratio*4),
                                           norm_layer(embed_dim_ratio*4),
    ...
    def TTE_foward(self, x):                                                        # x: (B*17, 243, 512)
        ...
        x = self.Temporal_norm[0](x)                                                   # x: (B*17,243,512)
        return x
...
class  Skeleton_in_Context(MixSTE2):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        self.temp_conv_query = nn.ModuleList([
            nn.Conv2d(embed_dim_ratio, embed_dim_ratio*2, kernel_size=(2,1)),
            nn.Conv2d(embed_dim_ratio*2, embed_dim_ratio*4, kernel_size=(4,1)),
            nn.Conv2d(embed_dim_ratio*4, embed_dim_ratio*4, kernel_size=(8,1)),
            nn.Identity(),
            nn.ConvTranspose2d(embed_dim_ratio*4, embed_dim_ratio*4, kernel_size=(8,1)),
            nn.ConvTranspose2d(embed_dim_ratio*4, embed_dim_ratio*2, kernel_size=(4,1)),
            nn.ConvTranspose2d(embed_dim_ratio*2, embed_dim_ratio, kernel_size=(2,1))
        ])

        self.temp_conv_prompt = nn.ModuleList([
            nn.Conv2d(embed_dim_ratio, embed_dim_ratio*2, kernel_size=(2,1)),
            nn.Conv2d(embed_dim_ratio*2, embed_dim_ratio*4, kernel_size=(4,1)),
            nn.Conv2d(embed_dim_ratio*4, embed_dim_ratio*4, kernel_size=(8,1)),
            nn.Identity(),
    ...
    def ST_foward_i(self, x, i):
        ...
        x = rearrange(x, 'b f n cw -> (b f) n cw')
        x = steblock(x)
        x = self.Spatial_norm[i+1](x)

        x = rearrange(x, '(b f) n cw -> (b n) f cw', f=f)
        x = tteblock(x)
        x = self.Temporal_norm[i+1](x)
        x = rearrange(x, '(b n) f cw -> b f n cw', n=n)
        return x
    ...
    def forward(self, Prompt, Query, epoch=None):                    # query_input: (B,243,17,3)
        ...
        for i in range(0, self.depth - 1):
            ...
            PROMPTS = PROMPTS + (prompt,)
            prompt = self.temp_conv_prompt[i](prompt.permute(0,3,1,2)).permute(0,2,3,1)
        ...
        for i in range(0, self.depth - 1):
            ...
            query += PROMPTS[i+3]
            query = self.temp_conv_query[i](query.permute(0,3,1,2)).permute(0,2,3,1)
