...
class Skeleton_in_Context(MixSTE2):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        self.ts_attn = nn.ModuleList([nn.Linear(embed_dim_ratio*2, 2) for _ in range(self.depth + 2)])
    ...
    def ST_foward(self, x, residual_x, ts_attn):
        assert len(x.shape)==4, "shape is equal to 4"
        b, f, n, cw = x.shape
        for i in range(1, self.block_depth):
            steblock = self.STEblocks[i]
            ...
            if residual_x is not None:
                alpha = ts_attn[i-1](torch.cat([x, residual_x[i-1]], dim=-1)).softmax(dim=-1)     # (B,243,17,2)
                x = x * alpha[..., 0:1] + residual_x[i-1] * alpha[...,1:2]
    ...
    def forward(self, Prompt, Query, epoch=None):                    # query_input: (B,243,17,3)
        ...
        alpha = self.ts_attn[0](torch.cat([query, PROMPTS[0]], dim=-1)).softmax(dim=-1)     # (B,243,17,2)
        query = query * alpha[..., 0:1] + PROMPTS[0] * alpha[...,1:2]


        query = self.STE_forward(query)                                                                         # query: (B,243,17,512)
        alpha = self.ts_attn[1](torch.cat([query, PROMPTS[1]], dim=-1)).softmax(dim=-1)     # (B,243,17,2)
        query = query * alpha[..., 0:1] + PROMPTS[1] * alpha[...,1:2]

        query = self.TTE_foward(query)                                                                          # query: (B,243,17,512)
        alpha = self.ts_attn[2](torch.cat([query, PROMPTS[2]], dim=-1)).softmax(dim=-1)     # (B,243,17,2)
        query = query * alpha[..., 0:1] + PROMPTS[2] * alpha[...,1:2]
        
        query = self.ST_foward(query, PROMPTS[3:], self.ts_attn[3:])
