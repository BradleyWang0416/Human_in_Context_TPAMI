...
class Skeleton_in_Context(nn.Module):
    ...
    def forward(self, Query, epoch=None):
        B, F, J, C = Query.shape # (B,F,17,3)

        Query_embed = self.encode(Query)  # (B,F,17,512)

        x = Query_embed.reshape(B * F, J, self.dim_feat)  # (B*F,17,512)

        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            x_st = blk_st(x, F)     # (B*F,17,512)
            x_ts = blk_ts(x, F)     # (B*F,17,512)
            if self.att_fuse:
        ...
        y = self.head(x)            # (B,F,17,512) --Linear(512,3)--> (B,F,17,3)
