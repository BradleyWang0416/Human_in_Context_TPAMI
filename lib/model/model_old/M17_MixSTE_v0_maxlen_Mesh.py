...
class Skeleton_in_Context(MixSTE2):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        self.mesh_head = SMPLRegressor('third_party/motionbert/data/mesh', embed_dim_ratio, num_joints, hidden_dim=1024, dropout_ratio=0.1)
    ...
    def forward(self, Prompt, Query, epoch=None):                    # query_input: (B,243,17,3)
        ...
        # 3D Branch
        query_output_3D = self.head(query)
        query_output_3D = query_output_3D.view(b, f, n, -1)


        # Mesh Branch
        query_output_smpl = self.mesh_head(query)
        for s in query_output_smpl:
            s['theta'] = s['theta'].reshape(b, f, -1)
            s['verts'] = s['verts'].reshape(b, f, -1, 3)
            s['kp_3d'] = s['kp_3d'].reshape(b, f, -1, 3)

        return query_output_3D, query_output_smpl