...
class Skeleton_in_Context(MixSTE2):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        self.context_head = ActionHeadClassification(dropout_ratio=0.5, dim_rep=embed_dim_ratio, num_classes=num_class, num_joints=num_joints, hidden_dim=2048)
    ...
    def forward(self, Prompt, Query, epoch=None):                    # query_input: (B,243,17,3)
        ...
        # MERGE
        query_output = self.head(query)

        query_context = self.context_head(query)    # (b, num_classes)

        query_output = query_output.view(b, f, n, -1)
