...
class Skeleton_in_Context(nn.Module):
    def __init__(self, args, dim_in=3, dim_out=3, dim_feat=512, dim_rep=512, depth=5, num_heads=8, mlp_ratio=2,
        ...
        ########################################################################################
        # self.TUP_ST = nn.Parameter(torch.zeros(1, maxlen, num_joints, 1))   # (1,F,17,1)
        # self.TUP_C = nn.Parameter(torch.zeros(1, self.dim_feat))   # (1,512)
    ...
    def generate_mask(self, C1, C2):
        ...
        if self.mask_type == "mask_half":
            ...
            vis = torch.zeros(1, 1, 1, C1)
            mask = torch.ones(1, 1, 1, C2)
            bool_masked_pos = torch.cat([vis, mask], dim=-1).to(torch.bool) # (1, 1, 1, C)
        else:
    ...
    def forward(self, Prompt, Query, epoch=None):
        if len(Query.shape) == 4:
            ...
            f = F//2
            query_input = Query[:, :f, :, :]     # (B,f,J,C)
            # query_target = torch.zeros_like(Query[:, f:, :, :]).cuda()            
            prompt_input = Prompt[:, :f, :, :]   # (B,f,J,C)
            prompt_target = Prompt[:, f:, :, :]  # (B,f,J,C)
            query = torch.cat([query_input, query_input], dim=-1)  # (B,f,J,2C)
            prompt = torch.cat([prompt_input, prompt_target], dim=-1)  # (B,f,J,2C)
            if_ntu60 = False
        ...
        query_embed = self.encode(query)        # (B,f,J,512)
        prompt_embed = self.encode(prompt)      # (B,f,J,512)

        x = torch.cat((prompt_embed, query_embed), dim=0)  # (2B,f,J,512)
        x = x.reshape(2 * B * f, J, self.dim_feat)  # (2B*f,J,512)

        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            if idx == self.merge_idx:
                x = x.reshape(2 * B,  f, J, self.dim_feat)  # (2B,f,J,512)
                x = (x[:B] + x[B:]) * 0.5   # (B,f,J,512)
                x = x.reshape(B * f, J, self.dim_feat)  # (Bf,J,512)
            x_st = blk_st(x, f)     # (2B*f,J,512) -- before merge; (B*f,J,512) -- after merge
            x_ts = blk_ts(x, f)     # (2B*f,J,512) -- before merge; (B*f,J,512) -- after merge
            if self.att_fuse:
                ...
                x = (x_st + x_ts)*0.5
        x = self.norm(x)            # (B*f,J,512)
        x = x.reshape(B, f, J, -1)  # (B,f,J,512)
