...
class Attention(nn.Module):
    ...
    def forward(self, x, seqlen=1, x2=None):
        B, N, C = x.shape
        ...
        if self.mode == 'series':
            ...
            q = self.to_q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)         # (B, head, N=17, C//head)
            
            x2 = x2 if x2 is not None else x
            k = self.to_k(x2).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)        # (B, head, N=17, C//head)
            v = self.to_v(x2).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)        # (B, head, N=17, C//head)
...
class CrossBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_out_ratio=1., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.cross_norm_s = norm_layer(dim)
        self.cross_norm_t = norm_layer(dim)
    ...
    def forward(self, x, seqlen=1, x2=None):
        # x: (BF,17,512); seqlen=F (number of frames, 243 for pose esti and action recog; 16 for mesh recov)
        if self.st_mode=='stage_st':
            x = x + self.drop_path(self.attn_s(self.norm1_s(x), seqlen, self.cross_norm_s(x2)))     # x: (BF,17,512)
            x = x + self.drop_path(self.mlp_s(self.norm2_s(x)))     # x: (BF,17,512)
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen, self.cross_norm_t(x2)))     # x: (BF,17,512)
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))     # x: (BF,17,512)
        elif self.st_mode=='stage_ts':
            x = x + self.drop_path(self.attn_t(self.norm1_t(x), seqlen, self.cross_norm_t(x2)))     # x: (BF,17,512)
            x = x + self.drop_path(self.mlp_t(self.norm2_t(x)))     # x: (BF,17,512)
...
class Skeleton_in_Context(nn.Module):
    def __init__(self, args, dim_in=3, dim_out=3, dim_feat=512, dim_rep=512, depth=5, num_heads=8, mlp_ratio=2,
        ...
        self.blocks_ts = nn.ModuleList([
            Block(
                ...
                st_mode="stage_ts")
            for i in range(depth)])
        self.cross_blocks_st = nn.ModuleList([
            CrossBlock(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode="stage_st")
            for i in range(depth)])
        self.cross_blocks_ts = nn.ModuleList([
            CrossBlock(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode="stage_ts")
            for i in range(depth)])
        self.blocks_st_forprompt = nn.ModuleList([
            Block(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode="stage_st")
            for i in range(depth)])
        self.blocks_ts_forprompt = nn.ModuleList([
            Block(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode="stage_ts")
        ...
        ########################################################################################
        self.TUP_ST = nn.Parameter(torch.zeros(1, 2*maxlen, num_joints, 1))   # (1,2F,17,512)
        self.TUP_C = nn.Parameter(torch.zeros(1, self.dim_feat))   # (1,2F,17,512)
        self.merge_idx = args.get("merge_idx", 2)
        self.mask_type = "mask_half"
        self.clip_len_2DAR = args.data.get("clip_len_2DAR", args.data.clip_len)
        self.merge_double_action = args.get("merge_double_action", "no")
        ########################################################################################
        
        self.joints_embed_prompt = nn.Linear(dim_in, dim_feat)
        self.temp_embed_prompt = nn.Parameter(torch.zeros(1, 2*maxlen, 1, dim_feat))
        self.pos_embed_prompt = nn.Parameter(torch.zeros(1, num_joints, dim_feat))
    ...
    def encode_prompt(self, x):
        ...
        BF = x.shape[0]
        x = self.joints_embed_prompt(x)  # (BF,17,3) --nn.Linear(3,dim_feat=512)--> (BF,17,512)
        x = x + self.pos_embed_prompt  # (BF,17,512)
        _, J, C = x.shape
        x = x.reshape(-1, F, J, C) + self.temp_embed_prompt[:, :F, :, :]  # (B,F,17,512)
    ...
    def generate_mask(self, F1, F2):
        '''
        function: generate mask (0-visible, 1-invisible).
        return: (B,2F) type(bool)
        '''
        if self.mask_type == "mask_half":
            # generate mask 训练和测试时mask的策略相同，都是mask后半部分
            vis = torch.zeros(1, F1, 1, 1)
            mask = torch.ones(1, F2, 1, 1)
            bool_masked_pos = torch.cat([vis, mask], dim=1).to(torch.bool) # (1, F, 1, 1)
        else:
    ...
    def forward(self, Prompt, Query, epoch=None):
        if len(Query.shape) == 4:
            B, F, J, C = Query.shape # B: batch_size, F: total number of frames, J: 17 joints, C: 3 dims
            F1 = F // 2
            F2 = F // 2
            if_ntu60 = False
        elif len(Query.shape) == 5:
            B_, M, F, J, C = Query.shape
            F1 = self.clip_len_2DAR
            F2 = F - F1

            Query = Query.reshape(B_*M, F, J, C)
            Prompt = Prompt.reshape(B_*M, F, J, C)
            B = B_*M
            if_ntu60 = True

        Query_embed = self.encode(Query)  # (B,F,17,dim_feat)
        Prompt_embed_for_crossattn = self.encode_prompt(Prompt)    # (B,F,17,dim_feat)

        bool_masked_pos = self.generate_mask(F1, F2) # (1, F, 1, 1). elements: [False, False, ..., True, True]

        TUP = torch.einsum('btjk,kc->btjc', self.TUP_ST, self.TUP_C)
        TUP = TUP.expand(B, -1, -1, -1)

        m = bool_masked_pos.type_as(TUP).expand(B, -1, -1, -1)  # (B,F,1,1). elements: [0, 0, ..., 1, 1]

        Query_embed = Query_embed * (1 - m) + TUP * m   # (B,F,17,dim_feat)
        
        x = Query_embed.reshape(B * F, J, self.dim_feat)  # (B*F,17,dim_feat)
        Prompt_embed_for_crossattn = Prompt_embed_for_crossattn.reshape(B * F, J, self.dim_feat)
        prompt_x = Prompt_embed_for_crossattn

        for idx, (blk_st, blk_ts, cross_blk_st, cross_blk_ts, blk_st_forprompt, blk_ts_forprompt) in enumerate(zip(self.blocks_st, self.blocks_ts, self.cross_blocks_st, self.cross_blocks_ts, self.blocks_st_forprompt, self.blocks_ts_forprompt)):
            # x_st = blk_st(x, F)     # (B*F,17,dim_feat)
            # x_ts = blk_ts(x, F)     # (B*F,17,dim_feat)
            
            # x_st = cross_blk_st(x_st, F, Prompt_embed_for_crossattn)
            # x_ts = cross_blk_ts(x_ts, F, Prompt_embed_for_crossattn)
            
            # if self.att_fuse:
            #     att = self.ts_attn[idx]     # nn.Linear(dim_feat*2, 2)
            #     alpha = torch.cat([x_st, x_ts], dim=-1)     # (B*F,17,dim_feat*2)
            #     BF, J = alpha.shape[:2]
            #     alpha = att(alpha)  # (B*F,17,2)
            #     alpha = alpha.softmax(dim=-1)   # (B*F,17,2)
            #     x = x_st * alpha[:,:,0:1] + x_ts * alpha[:,:,1:2]   # (B*F,17,dim_feat)
            # else:
            #     x = (x_st + x_ts)*0.5
            ...
            prompt_x_st = blk_st_forprompt(prompt_x, F)
            prompt_x_ts = blk_ts_forprompt(prompt_x, F)
            
            x_st = cross_blk_st(x_st_, F, prompt_x_st)
            x_ts = cross_blk_ts(x_ts_, F, prompt_x_ts)
            
            prompt_x = (prompt_x_st + prompt_x_ts) * 0.5
            ...
            # x = (cross_blk_st(x_st, F, x_ts) + cross_blk_ts(x_ts, F, x_st)) * 0.5
            # x = cross_blk_st(x, F, Prompt_embed_for_crossattn)
            # x = cross_blk_ts(x, F, Prompt_embed_for_crossattn)
            
            
        x = self.norm(x)            # (B*F,17,dim_feat)
        x = x.reshape(B, F, J, -1)  # (B,F,17,dim_feat)

        if if_ntu60: 
            x = x.reshape(B_, M, F, J, -1)

        if if_ntu60 and self.merge_double_action == 'before_pre_logits':
            x = x.mean(1, keepdim=True) 
        x = self.pre_logits(x)      # (B,F,17,dim_feat*2) --Linear(dim_feat*2, dim_rep)--> --tanh--> (B,F,17,dim_rep)

        if if_ntu60 and self.merge_double_action == 'before_head':
            x = x.reshape(B_, M, F, J, -1)
            x = x.mean(1, keepdim=True) 
        x = self.head(x)            # (B,F,17,dim_rep) --Linear(dim_rep,3)--> (B,F,17,3)

        rebuild_part = x[..., F1:, :, :]  # (B, F2, 17, 3)
        if if_ntu60 and self.merge_double_action == 'before_head':
            rebuild_part = rebuild_part.reshape(B_, M, F2, J, -1)
            rebuild_part = rebuild_part.mean(1, keepdim=True)
        return rebuild_part
