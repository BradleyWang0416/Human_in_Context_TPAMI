...
class Skeleton_in_Context(MixSTE2):
    ...
    def forward(self, Prompt, Query, epoch=None):                    # query_input: (B,243,17,3)
        ...
        if self.training:
            mask = torch.rand(query_input.shape[0], query_input.shape[1], 1, 1, device=query_input.device) > 0.5
        ...
        query_out_list = self.ST_foward(query, PROMPTS[4:])


        # MERGE
        query_out = self.head(query_out_list[-1])
        query_out = query_out.view(b, f, n, -1)

        # Second Output
        query_recon = self.recon_head(query_out_list[1])
        query_recon = query_recon.view(b, f, n, -1)
