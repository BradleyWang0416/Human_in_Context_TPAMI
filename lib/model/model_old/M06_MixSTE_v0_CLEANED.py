...
class  Skeleton_in_Context(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, hidden_dim=32, depth=4, num_heads=8, mlp_ratio=2, 
                 qkv_bias=True, qk_scale=None, drop_rate=0, attn_drop_rate=0, drop_path_rate=0.2, norm_layer=None):
        super().__init__()

        self.depth = depth

        self.Query_spatial_pos_embed = nn.Parameter(torch.zeros(num_joints, hidden_dim))
        self.Prompt_spatial_pos_embed = nn.Parameter(torch.zeros(num_joints, hidden_dim))

        self.Query_spatial_patch_to_embedding = nn.Linear(in_chans + 3, hidden_dim)
        self.Prompt_spatial_patch_to_embedding = nn.Linear(in_chans + 3, hidden_dim)

        self.Query_ste = STE(hidden_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
        self.Query_tte = TTE(num_frame, hidden_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
        self.Query_st = ST(hidden_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)

        self.Prompt_ste = STE(hidden_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
        self.Prompt_tte = TTE(num_frame, hidden_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
        self.Prompt_st = ST(hidden_dim, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
    ...
    def forward(self, Prompt, Query, epoch=None):                    # query_input: (B,243,17,3)
        ...
        # QUERY branch
        query = self.query_encode_forward(query_input, prompt_groundtruth_output)                                         # query: (B,243,17,512)
        query += PROMPTS[0]

        query = self.Query_ste(query)                                                                         # query: (B,243,17,512)
        query += PROMPTS[1]

        query = self.Query_tte(query)                                                                          # query: (B,243,17,512)
        query += PROMPTS[2]
        
        for i in range(self.depth - 1):
            query = self.Query_st.forward_i(query, i)
            query += PROMPTS[3+i]

        # MERGE
        query = self.head(query)

        query = query.view(B, F1, J, -1)
