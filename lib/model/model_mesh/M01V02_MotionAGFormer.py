...
class Skeleton_in_Context(BASE_CLASS):
    def __init__(self, args, num_frame=9, num_joints=17, in_chans=3, hidden_dim=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        query_input_dim = in_chans+3 if not args.deactivate_prompt_branch else in_chans
        self.spatial_patch_to_embedding = nn.ModuleDict({'query': nn.Linear(query_input_dim, hidden_dim),
                                                         'prompt': nn.Linear(in_chans+3, hidden_dim)})

        # self.ste = nn.ModuleDict({'query': STE(dim_feat, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer),
        #                           'prompt': STE(dim_feat, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)})
        
        # self.tte = nn.ModuleDict({'query': TTE(num_frame, dim_feat, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer),
        #                           'prompt': TTE(num_frame, dim_feat, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)})
        
        # self.st = nn.ModuleDict({'query': ST(dim_feat, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer),
        #                          'prompt': ST(dim_feat, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)})
    ...
    def encode_joint(self, x_in, x_out=None, key=None):
        x = torch.cat([x_in, x_out], dim=-1) if x_out is not None else x_in
    ...
    def forward(self, query_input_dict,
        ...
        if not deactivate_prompt_branch:
        ...
        if not deactivate_prompt_branch: 
            ...
            query = self.encode_joint(query_input, key='query')     # [B,T,17,3] || [B,T,17,3] --> [B,T,17,6] --> [B,T,17,512]


        for i, layer in enumerate(self.MotionAGFormer['query'].layers):
            query = layer(query)
            if not deactivate_prompt_branch: query += PROMPTS[1+i]
        ...
        output_joint = self.joint_head(output_smpl[-1]['verts'])
        if vertex_x1000:
