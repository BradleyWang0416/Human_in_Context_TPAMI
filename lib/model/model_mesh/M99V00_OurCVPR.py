...
class Skeleton_in_Context(BASE_CLASS):
    ...
    def forward(self, query_input_dict,
        ...
        B, F1, J = query_input_dict['joint'].shape[:3]
        F2 = F1
        F = F1 * 2

        query_input = query_input_dict['joint']     # [B,F1,17,3]
        prompt_input = prompt_input_dict['joint']   # [B,F1,17,3]
        prompt_target_joint = prompt_target_dict['joint']       # [B,F1,17,3]

        Query = torch.cat([query_input, target_joint], dim=1)  # (B,F,17,3)
        Prompt = torch.cat([prompt_input, prompt_target_joint], dim=1)  # (B,F,17,3)
        ...
        query = x[:, F1:, :, :]  # (B,F2,17,512)

        output_smpl = self.smpl_head(query, vertex_x1000=vertex_x1000)
        for s in output_smpl:
            s['theta'] = s['theta'].reshape(B, F1, -1)
            s['verts'] = s['verts'].reshape(B, F1, -1, 3)
            s['kp_3d'] = s['kp_3d'].reshape(B, F1, -1, 3)
