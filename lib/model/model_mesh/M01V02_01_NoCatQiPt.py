...
class Skeleton_in_Context(BASE_CLASS):
    def __init__(self, args, num_frame=9, num_joints=17, in_chans=3, hidden_dim=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        self.spatial_patch_to_embedding = nn.ModuleDict({'query': nn.Linear(in_chans, hidden_dim),
    ...
    def forward(self, query_input_dict,
        ...
        # QUERY BRANCH
        query = self.encode_joint(query_input, None, 'query')     # [B,T,17,3] || [B,T,17,3] --> [B,T,17,6] --> [B,T,17,512]
        if not deactivate_prompt_branch: query += PROMPTS[0]
