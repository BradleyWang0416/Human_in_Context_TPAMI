...
from third_party.motionbert.lib.data.augmentation import Augmenter2D, Augmenter3D
...
class Skeleton_in_Context(BASE_CLASS):
    def __init__(self, args, num_frame=9, num_joints=17, in_chans=3, hidden_dim=32, depth=4, num_heads=8, mlp_ratio=2, 
        ...
        setattr(args, 'd2c_params_path', 'third_party/motionbert/params/d2c_params.pkl')
        setattr(args, 'noise_path', 'third_party/motionbert/params/synthetic_noise.pth')
        setattr(args, 'mask_ratio', 0.05)
        setattr(args, 'mask_T_ratio', 0.1)
        self.augmenter2D = Augmenter2D(args)
    ...
    def forward(self, query_input_dict,
        ...
        x = self.augmenter2D.augment2D(query_input, noise=True, mask=True)

        x = self.encode(x)  # (B,F,17,512)
        x = x.reshape(B * F1, J, self.dim_feat)  # (2B*F,17,512)

        for idx, (blk_st, blk_ts) in enumerate(zip(self.blocks_st, self.blocks_ts)):
            x_st = blk_st(x, F1)     # (2B*F,17,512) -- before merge; (B*F,17,512) -- after merge
            x_ts = blk_ts(x, F1)     # (2B*F,17,512) -- before merge; (B*F,17,512) -- after merge
            if self.att_fuse:
        ...
        x = x.reshape(B, F1, J, -1)  # (B,F,17,512)


        x = self.pre_logits(x)      # (B,F,17,512) --Linear(dim_feat=512, dim_rep=512)--> --tanh--> (B,F,17,512)

        output_smpl = self.smpl_head(x, vertex_x1000=vertex_x1000)
