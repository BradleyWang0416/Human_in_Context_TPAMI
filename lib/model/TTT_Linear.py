...
from lib.model.M06_MixSTE_v0 import STE, TTE, ST
...
class TTTLinear(TTTBase):
    def __init__(self, num_heads=8, hidden_size=512, mini_batch_size=64, rope_theta=10000):
...
# class Skeleton_in_Context():
#     def __init__(self, num_heads=8, hidden_size=512, mini_batch_size=64, rope_theta=10000, 
#                  num_frame=16, mlp_ratio=2, qkv_bias=True, qk_scale=None, drop_rate=0, attn_drop_rate=0, depth=8, drop_path_rate=0.1, norm_layer=partial(nn.LayerNorm, eps=1e-6)):
        
#         self.ttt_linear = TTTLinear(num_heads, hidden_size, mini_batch_size, rope_theta)

#         self.position_ids = torch.arange(num_frame)

#         self.query_spatial_patch_to_embedding = nn.Linear(6, hidden_size)
#         self.query_spatial_pos_embed = nn.Parameter(torch.zeros(17, hidden_size))
#         self.prompt_spatial_patch_to_embedding = nn.Linear(6, hidden_size)
#         self.prompt_spatial_pos_embed = nn.Parameter(torch.zeros(17, hidden_size))

#         self.query_ste = STE(num_frame, hidden_size, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
#         self.query_tte = TTE(num_frame, hidden_size, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)

#         self.prompt_ste = STE(num_frame, hidden_size, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
#         self.prompt_tte = TTE(num_frame, hidden_size, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, depth, drop_path_rate, norm_layer)
...
#         prompt = self.prompt_encode_forward(prompt_input, prompt_groundtruth_output)        # (b,f,j,hidden_size)
#         prompt = self.prompt_ste(prompt)
#         prompt = self.prompt_tte(prompt) 

#         query = self.query_encode_forward(query_input, prompt_groundtruth_output)           # (b,f,j,hidden_size)
...
#         position_ids = self.position_ids.unsqueeze(0).expand(b, -1)
#         cache = TTTCache(model=self.ttt_linear, batch_size=32, mini_batch_size=64)

#         """ttt_linear.forward arguments
#         input_tensor: (B,T,C)
#         position_ids: (B,T)
#         """
#         return self.ttt_linear(input_tensor, position_ids, cache)
