...
input_length = 32           # default: 16
train_shift = 2             # default: 1
test_shift = 4              # default: 4
drop_ratios = [0.4,0.6]     # default: [0.4, 0.6]
############################# customize #############################

if use_default_setting:
    input_length, train_shift, test_shift, drop_ratios = 16, 1, 4, [0.4, 0.6]
    save_root_path = "data/3DPW_MC"
else:
    save_root_path = "data/non_default/3DPW_MC"
    save_folder = f'TrainShift{train_shift}_TestShift{test_shift}_DropRatios{int(drop_ratios[0]*10)}{int(drop_ratios[1]*10)}_ClipLen{input_length}'
    ...
    def _collect_all(self):
        ...
        for pw3d_seq_name in tqdm(self._pw3d_file_names):
            ...
            for i in range(len(pw3d_motion_poses)):
                N = len(pw3d_motion_poses[i])                                   # (1918,72)

                sampled_index = np.arange(0, N, sample_rate)
                motion_poses = pw3d_motion_poses[i][sampled_index]              # (959,72)
