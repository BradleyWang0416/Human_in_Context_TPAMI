...
def main():
    ...
    amass_input_length = 16     # default: 10
    amass_target_length = 16    # default: 10
    test_shift = 40             # default: 20
    train_shift = 40            # default: 20
    JOINT_NUMBER = 22               # default: 18
    FPS = 25                        # default: 25
    ############################# customize #############################

    if use_default_setting:
        clip_len, amass_input_length, amass_target_length, test_shift, train_shift, JOINT_NUMBER, FPS = 16, 10, 10, 20, 20, 18, 25
        save_root_path = f"data/AMASS"
    else:
        save_root_path = 'data/non_default_ICL/AMASS/'
        save_folder = f'TrainShift{train_shift}_TestShift{test_shift}_ClipLen{clip_len}_In{amass_input_length}_Out{amass_target_length}_J{JOINT_NUMBER}_FPS{FPS}'
    ...
    # --------------------------------------------- Train --------------------------------------------------------------------------
    amass_dataset = AMASSDataset('train', amass_anno_dir, amass_support_dir, amass_input_length, amass_target_length, motion_dim, train_shift, JOINT_NUMBER=JOINT_NUMBER, FPS=FPS)
    ...
    for i in range(len(amass_dataset)):
        amass_motion = amass_dataset[i]
        data_dict = {
            "chunk_3d": amass_motion.data.numpy()
        }
        # angle = vector_angle(np.array([1.,0.]), amass_motion[0, 1, [0,2]]-amass_motion[0, 2, [0,2]])
        # ANGLES.append(np.abs(angle-360))
    ...
    # --------------------------------------------- Test --------------------------------------------------------------------------
    amass_dataset_test = AMASSEval('test', amass_anno_dir, amass_support_dir, amass_input_length, amass_target_length, motion_dim, test_shift, JOINT_NUMBER=JOINT_NUMBER, FPS=FPS)
    ...
    for i in range(len(amass_dataset_test)):
        amass_motion = amass_dataset_test[i]
        data_dict = {
            "chunk_3d": amass_motion.data.numpy()
        }
        # with open(os.path.join(save_path, "%08d.pkl" % i), "wb") as myprofile:
...
class AMASSEval(data.Dataset):
    def __init__(self, split_name, amass_anno_dir, support_dir, amass_input_length, amass_target_length, motion_dim, shift_step, JOINT_NUMBER, FPS):
    ...
    def _collect_all(self):
        ...
        for amass_seq_name in tqdm(self._amass_file_names):
            ...
            sample_rate = int(frame_rate // self.FPS)    
            ...
            # amass_motion_poses_22joints = amass_motion_poses.clone().reshape(-1, 52, 3)[:, :22]     # [T, 22, 3]
            # amass_motion_poses_18joints = amass_motion_poses.clone().reshape(-1, 52, 3)[:, 4:22]     # [T, 18, 3]
            # viz_skel_seq_anim(data={"amass_22": amass_motion_poses_22joints[:], 
            #                         "amass_18": amass_motion_poses_18joints[:]}, subplot_layout=(1, 2), fs=0.6, azim=0, elev=70, special_node=3, if_node=True)
            
            if self.JOINT_NUMBER == 18:
                amass_motion_poses = amass_motion_poses.reshape(-1, 52, 3)[:, 4:22].reshape(T, 54)
            elif self.JOINT_NUMBER == 22:
                amass_motion_poses = amass_motion_poses.reshape(-1, 52, 3)[:, :22].reshape(T, 66)
    ...
    def __getitem__(self, index):
        ...
        motion = motion.reshape(self.amass_motion_input_length + self.amass_motion_target_length, self.JOINT_NUMBER, 3)
...
class AMASSDataset(data.Dataset):
    def __init__(self, split_name, amass_anno_dir, support_dir, amass_input_length, amass_target_length, motion_dim, train_shift, data_aug=True, JOINT_NUMBER=18, FPS=25):
    ...
    def __getitem__(self, index):
        if self.shift_step is not None:
            ...
            motion = amass_motion.reshape(self.amass_motion_input_length+self.amass_motion_target_length, self.JOINT_NUMBER, 3)
