...
def train_with_config(args, opts):
    ...
    #     print('\nCUDA_VISIBLE_DEVICES='+os.environ.get('CUDA_VISIBLE_DEVICES'), end=' ')
    # else:
    #     print('\n', end='')
    ...
    try:
        ...
        if e.errno != errno.EEXIST:
            raise RuntimeError('Unable to create checkpoint directory:', os.path.join(opts.checkpoint, 'best_epochs'))
    ...
    if len([task for task in args.tasks if task not in ['AR']]) > 0:
        ...
        train_loader['non_AR'] = DataLoader(train_dataset, **trainloader_params)
    ...
    if 'AR' in args.tasks:
        ...
        train_loader['AR'] = DataLoader(**train_loader_params_AR)
    ...
    if dataset_VER in ['2_ICL']:
        for (dataset, task) in test_dataset.keys():
            test_loader[(dataset, task)] = DataLoader(test_dataset[(dataset, task)], **testloader_params)
    ...
    if not opts.evaluate:
        ...
        if not args.no_eval:
            ...
            # for task in args.tasks:
            #     if task in ['AR', '2DAR']:
            #         print(f"{task} \t | \t {eval_dict[task]['max_acc']}% (epoch {eval_dict[task]['best_epoch']})")
            #     else:
